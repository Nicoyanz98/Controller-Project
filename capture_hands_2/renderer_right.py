...
class RendererRight:
    ...
    def draw_title(self):
        title = self.font.render("Lado Derecho - Stick + A/B/X/Y", True, AppConfig.TEXT)
    ...
    def draw_sticks_display(self):
        ...
        title_x = AppConfig.SCREEN_WIDTH // 2 - sticks_title.get_width() // 2
        self.screen.blit(sticks_title, (title_x, start_y - 30))


        stick_positions = ['stick_right_up_left', 'stick_right_up', 'stick_right_up_right',
                           'stick_right_left', 'stick_right_neutral', 'stick_right_right', 
                           'stick_right_down_left', 'stick_right_down', 'stick_right_down_right'
        ]

        for i, pos in enumerate(stick_positions):
            direction = pos.replace('stick_right_', '')
            stick_img = self.stick_images_pressed.get(direction) if self.button_states[pos] else self.stick_images_normal.get(direction)
            ...
            if stick_img:
                ...
                self.button_rects[pos] = pygame.Rect(x_pos, y_pos, 60, 60)


    def draw_gamepad_buttons(self):
        camera_frame = self.app.camera_system.get_frame()
        ...
        buttons_title = self.font.render("Botones A/B/X/Y", True, AppConfig.TEXT)
        title_x = AppConfig.SCREEN_WIDTH // 2 - buttons_title.get_width() // 2
        self.screen.blit(buttons_title, (title_x, start_y - 30))

        button_positions = ['button_Y', 'button_X', 'button_A', 'button_B']
    ...
    def draw_button_pressed_feedback(self):
        ...
        if button_img:
            ...
            button_label = active_button.replace('_', ' ').title()
    ...
    def load_button_images(self, state):
        ...
        # Mapeo de nombres de botones a nombres de archivos
        button_files = {
            # Sticks
